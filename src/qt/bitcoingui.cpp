...
{
    resize(850, 550);
    setWindowTitle(tr("goldeneagle") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a goldeneagle address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About goldeneagle"), this);
    aboutAction->setToolTip(tr("Show information about goldeneagle"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for goldeneagle"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("goldeneagle client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("goldeneagle client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to goldeneagle network.", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid goldeneagle address or malformed URI parameters."));
    }
